...
class AutoHypergraph(
    ...
    def _edge_payload(self, chunk: str, known_nodes: str) -> dict[str, Any]:
        """Build the two-stage hyperedge extractor payload for a single chunk."""
        return {"source_text": chunk, "known_nodes": known_nodes}

    def _empty_known_nodes_placeholder(self) -> str:
        """Placeholder text when a chunk produced no nodes for hyperedge extraction."""
        return "No entities identified in this chunk."

    def _format_known_nodes(self, nodes: list[NodeSchema]) -> str:
        """Format extracted node keys for the two-stage hyperedge prompt."""
        if not nodes:
    ...
    def _extract_data_by_two_stage(self, text: str) -> AutoHypergraphSchema:
        ...
        A chunk whose node extraction failed is skipped for hyperedge extraction
    ...
    def _extract_nodes_batch(
        ...
        Failed chunks are left as ``None`` so hyperedge extraction can skip them
